...
//        list13 = list13;
    ...
    /*
     * The following tests are created using an input space partitioning strategy for:
     * parameterize(final Class<?> rawClass, final Map<TypeVariable<?>, Type> typeVariableMap)
     * Variables: Class<?> rawClass, Map<TypeVariable<?>, Type> typeVariableMap
     */

    /*
     * Variable: rawClass
     * Characteristic: Is Parameterized
    ...
    public void testIsParameterized() {
        ...
        typeVariableMap.put(paramClass.class.getTypeParameters()[0], String.class);
        //True
        assertEquals(String.class, TypeUtils.parameterize(paramClass.class, typeVariableMap).getActualTypeArguments()[0]);

        ParameterizedType param = TypeUtils.parameterize(String.class, new HashMap<>());
        //False
        assertEquals(0, param.getActualTypeArguments().length);
    }

    /*
     * Variable: typeVariableMap
     * Characteristic: Contains correct TypeVariables
     * Partitions: True, False
     */
    @Test
    public void testMissingParameters() {
        //True partition already tested above
        //False
        assertThrows(RuntimeException.class, () -> {
            TypeUtils.parameterize(paramClass.class, new HashMap<>());
        });
    }
     ...
     * Variables: Type type, Class<?> toClass
     */

    /*
     * Variable: type
     * Characteristic: Superinterface of type
     * Partitions: Class<?>, ParameterizedType, GenericArrayType, WildcardType, TypeVariable<D>
     */
    class GenericComponentClass {
        public List<String>[] genericArray;
        public WildcardType wildType = new wildInner();

        class wildInner implements WildcardType {
    ...
    @Test
    public void testSuperinterfaceBlocks() {
        GenericComponentClass gen = new GenericComponentClass();

        //Class<?>
        assertEquals(null, TypeUtils.getTypeArguments(String.class, Class.class));

        //ParameterizedType
        assertEquals(Object.class, TypeUtils.getTypeArguments(Properties.class, Map.class).get(Map.class.getTypeParameters()[0]));
        assertEquals(Object.class, TypeUtils.getTypeArguments(Properties.class, Map.class).get(Map.class.getTypeParameters()[1]));

        //GenericArrayType
        try {
            Type genericArray = gen.getClass().getField("genericArray").getGenericType();
            assertEquals(String.class, TypeUtils.getTypeArguments(genericArray, List.class).get(List.class.getTypeParameters()[0]));
        } catch(Exception e) {}

        //WildcardType
        WildcardType wildType = gen.wildType;
        assertEquals(new HashMap<>(), TypeUtils.getTypeArguments(wildType, Object.class));

        //TypeVariable<D>
        TypeVariable type = new ArrayList<String>().getClass().getTypeParameters()[0];
        assertEquals(new HashMap<>(), TypeUtils.getTypeArguments(type, Object.class));
    }

    /*
     * Variable: type
     * Characteristic: Is a valid type
     * Partitions: True, False
     */
    @Test
    public void testIsValidType() {
        //True - has already been tested several times.

        //False - This is mostly to check that the IllegalStateException line can be hit.
        // I don't know of any types that aren't handled by this method, so I used null.
        assertThrows(IllegalStateException.class, () -> {
            System.out.println(TypeUtils.getTypeArguments(null, Object.class));
        });
    }

    /*
     * Variable: toClass
     * Characteristic: Is superinterface of type
     * Partitions: True, False
     */
    @Test
    public void testIsSuperinterface() {
        ...
        //False (I used the Wildcard type here to hit a missed branch)
        GenericComponentClass gen = new GenericComponentClass();
        assertNull(TypeUtils.getTypeArguments(gen.wildType, GenericArrayType.class));
    }


    /*
     * No strategy here. Just testing an untested hashCode method.
     */
    @Test
    public void testWildcardHash() {
        TypeUtils.WildcardTypeBuilder wildBuilder1 = TypeUtils.wildcardType(); // I really wish I had known about this before...
        WildcardType wild1 = wildBuilder1.build();

        TypeUtils.WildcardTypeBuilder wildBuilder2 = wildBuilder1.withUpperBounds(String.class);
        WildcardType wild2 = wildBuilder2.build();
        WildcardType wild3 = wildBuilder2.build();

        assertEquals(wild1.hashCode(), wild1.hashCode());
        assertNotEquals(wild1.hashCode(), wild2.hashCode());
        assertEquals(wild2.hashCode(), wild3.hashCode());

        TypeUtils.WildcardTypeBuilder wildBuilder3 = wildBuilder2.withLowerBounds(String.class);
        wild3 = wildBuilder3.build();

        assertNotEquals(wild2.hashCode(), wild3.hashCode());
    }

    /*
     * Again, no strategy here. Just testing an untested hashCode method.
     */
    @Test
    public void testParameterizedHash() {
        Type owner = TypeUtils.parameterize(TypeUtilsTest.class, String.class);
        Type otherOwner = TypeUtils.parameterize(TypeUtilsTest.class, Integer.class);
        ParameterizedType type1 = TypeUtils.parameterizeWithOwner(owner, That.class, String.class, String.class);
        ParameterizedType type2 = TypeUtils.parameterizeWithOwner(owner, That.class, String.class, Integer.class);
        ParameterizedType type3 = TypeUtils.parameterizeWithOwner(otherOwner, That.class, String.class, Integer.class);

        assertEquals(type1.hashCode(), type1.hashCode());
        assertNotEquals(type1.hashCode(), type2.hashCode());
        assertNotEquals(type2.hashCode(), type3.hashCode());
    }

    /*
     * This tests a missing branch on the parameterized type equals method
     */
    @Test
    ...
    /*
     * I noticed that the method, unrollBounds, had no test coverage but the method was private, so I identified a method that called unrollBounds and was public.
     * The method I discovered was unrollVariables which already had decent line coverage. Since there were several missed branches in this method, I decided to
     * use graph-based coverage to determine which nodes I could hit by tracing execution flow in order to find method inputs to get better branch and line coverage for both methods.
     */
    class ExtendedList<E> extends ArrayList {}
    @Test
    public void testUnrollVariables() {
        //Hit null check
        ParameterizedType checkType = TypeUtils.parameterize(ExtendedList.class, String.class);
        assertEquals(checkType, TypeUtils.unrollVariables(null, checkType));

        //Hit parameterized owner type != null branch
        Type owner = TypeUtils.parameterize(TypeUtilsTest.class, Other.class);
        ParameterizedType p = TypeUtils.parameterizeWithOwner(owner, That.class, Other.class, Other.class);
        owner = null;
        TypeVariable<?>[] variables = TypeUtilsTest.class.getTypeParameters();
        Map<TypeVariable<?>, Type> typeArguments = Collections.<TypeVariable<?>, Type>singletonMap(variables[0], Other.class);
        assertEquals(p.getClass(), TypeUtils.unrollVariables(typeArguments, p).getClass());

        //Hit WildcardType branch
        TypeUtils.WildcardTypeBuilder wildBuilder = TypeUtils.wildcardType();
        wildBuilder = wildBuilder.withUpperBounds(List.class);
        WildcardType wild = wildBuilder.build();
        assertEquals(wild.getClass(), TypeUtils.unrollVariables(null, wild).getClass());
    }

    //Simple test for a missed hashcode method in a Type class with no constructor.
    @Test
    public void testGenericArrayTypeHash() {
        Type type = new ArrayList<String>().getClass().getTypeParameters()[0];
        Type otherType = paramClass.class.getTypeParameters()[0];
        GenericArrayType genType = TypeUtils.genericArrayType(type);
        GenericArrayType genTypeTwo = TypeUtils.genericArrayType(type);
        GenericArrayType genTypeOther = TypeUtils.genericArrayType(otherType);

        assertEquals(genType.hashCode(), genType.hashCode());
        assertEquals(genType.hashCode(), genTypeTwo.hashCode());
        assertNotEquals(genType.hashCode(), genTypeOther.hashCode());
    }

    //Simple test for missed equals method in a Type class with no constructor
    @Test
    public void testGenericArrayTypeEquals() {
        ...
        assertEquals(genType, genType);
        assertEquals(genType, genTypeTwo);
        assertNotEquals(genType, genTypeOther);
    }

    /*
     * Here I used graph-based coverage to analyze the missing branches on a private method
     * called classToString(). I identified this as the best strategy since the method was
     * not directly callable and most of the challenge was just tracing missed branches through
     * other methods.
     */
    public class exampleClass<ExtendedList> {}
    @Test
    public void testClassToString() {
        Type type = exampleClass.class;
        String result = TypeUtils.toString(type);
        String expected = "org.apache.commons.lang3.reflect.TypeUtilsTest<B>.exampleClass<ExtendedList>";
        assertEquals(expected, result);
    }

    /*
     * I found a public method that had a ton of if statements to detect different types of inputs.
     * I decided that input space partitioning would be the best strategy here. Rather than assessing
     * every possible type of input, I chose to analyze which block combinations had been tested and which ones hadn't
     * so that I could hit all the missed branches.
     */
    ...
    /*
     * Variable: assigningType
     * Characteristic: Is Null
     * Partitions: True, False
     */
    @Test
    public void getRawTypeNullAssignType() {
        //True
        Type type = TestClass.class.getTypeParameters()[0];
        Type assigningType = null;
        assertNull(TypeUtils.getRawType(type, assigningType));

        //False
        assigningType = TestClass.class;
        assertNull(TypeUtils.getRawType(TestClass.class.getTypeParameters()[0], assigningType));
    }

    /*
     * Variable: type
     * Characteristic: Is WildcardType
     * Partitions: True, False
     */
    @Test
    public void getRawTypeWildcardType() {
        //True
        TypeUtils.WildcardTypeBuilder wildBuilder = TypeUtils.wildcardType();
        wildBuilder = wildBuilder.withUpperBounds(List.class);
        WildcardType wild = wildBuilder.build();
        assertNull(TypeUtils.getRawType(wild, wild));

        //False already hit in prior tests
    }
     ...
     */
    @Test
    public void getRawTypeNullType() {
        //True
        assertThrows(IllegalArgumentException.class, () -> {
           TypeUtils.getRawType(null, TestClass.class);
        });

        //False already hit in prior test
    }

    //Simple hashcode test for missing branch
    @Test
    public void testTypeEquals() {
        ...
        assertTrue(TypeUtils.equals(genType, genTypeTwo));
        assertFalse(TypeUtils.equals(genType, genTypeOther));

        TypeUtils.WildcardTypeBuilder wildBuilder = TypeUtils.wildcardType();
        wildBuilder = wildBuilder.withUpperBounds(List.class);
        WildcardType wild = wildBuilder.build();

        assertFalse(TypeUtils.equals(wild, genType));
    }

    //Test to hit the missing branches on isInstance
    @Test
    public void testIsInstanceMissingBranches() {
        assertFalse(TypeUtils.isInstance(null, null));
        assertTrue(TypeUtils.isInstance(null, Object.class));
        Integer value = 0;
        assertFalse(TypeUtils.isInstance(value, String.class));
    }
}
